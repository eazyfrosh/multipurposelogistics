...
import { getAllShipments, updateShipment } from "@/lib/services/shipments";
...
export default function AdminShipmentsPage() {
  ...
  async function applyBulkStatus(status: ShipmentStatus) {
    ...
    for (const s of targets) await updateShipment({ ...s, status });
    if (user && targets.length > 0) {
      await logActivity(user.uid, profile?.displayName ?? user.email, "bulk_status_update", "shipment", "-", `Updated ${targets.length} shipments to ${STATUS_LABELS[status]}`);
    }
    setSelected(new Set());
    reload();
  }
    ...
    <div>
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-foreground/55">Platform-wide shipment management.</p>
        </div>
        ...
        <div className="grid gap-3 sm:grid-cols-[1fr_auto_auto]">
          <div className="flex items-center gap-2 rounded-xl border border-black/12 px-3 dark:border-white/15">
            ...
            <input
              ...
              placeholder="Search tracking #, reference #, shipment #, sender, or receiver…"
      ...
      {selected.size > 0 && (
        <div className="mt-4 flex items-center gap-3 rounded-xl bg-brand-50 px-4 py-2.5 text-sm dark:bg-brand-500/10">
          ...
          <Button size="sm" variant="secondary" onClick={() => setBulkStatusOpen(true)}>Update status</Button>
          <Button size="sm" variant="ghost" onClick={() => exportShipmentsCsv(selectedShipments)}>Export selected</Button>
      ...
      <Card className="mt-4 overflow-x-auto">
              ...
              <th className="w-10 p-4">
                <input
                  ...
                  onChange={() =>
                    setSelected((prev) => {
                      const next = new Set(prev);
                      const all = paged.every((s) => next.has(s.id));
                      paged.forEach((s) => (all ? next.delete(s.id) : next.add(s.id)));
                      return next;
                    })
                  }
                />
            ...
            {!loading &&
              paged.map((s) => (
                <tr key={s.id} className="border-b border-black/6 last:border-0 hover:bg-black/[0.02] dark:border-white/8 dark:hover:bg-white/[0.02]">
                  ...
                  <td className="p-4">
                    <Link href={`/admin/shipments/${s.id}`} className="font-medium text-brand-600 hover:underline dark:text-brand-400">
                      {s.trackingNumber}
                    </Link>
        ...
        {!loading && filtered.length === 0 && (
          <EmptyState icon={<PackageSearch size={22} />} title="No shipments found" description="Try adjusting your filters." />
        )}
      </Card>
      ...
      <BulkStatusDialog open={bulkStatusOpen} onClose={() => setBulkStatusOpen(false)} count={selected.size} onApply={applyBulkStatus} />
    </div>
  );
}
