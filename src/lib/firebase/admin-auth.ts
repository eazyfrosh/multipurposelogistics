...
// Deliberately does not import firebase-admin/firestore. Consumers that only
// need to verify a caller's identity (e.g. the Blob upload/delete routes)
// should import from here rather than from ./admin, so their bundle doesn't
// pull in the Firestore SDK for no reason. firebase-admin/app's own app
// registry (getApps()) is global per process, so this safely shares a single
// initialized app with ./admin regardless of which module initializes it first.
...
export function getFirebaseAdminApp(): App {
  ...
  if (missing.length > 0) {
    throw new Error(`Firebase Admin SDK is not configured. Missing environment variable(s): ${missing.join(", ")}`);
  }
  ...
  try {
    ...
    throw new Error(`Firebase Admin SDK failed to initialize: ${err instanceof Error ? err.message : String(err)}`);
  }
